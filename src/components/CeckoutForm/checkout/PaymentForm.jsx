...
const PaymentForm = ({ checkoutToken, backStep, shippingData ,onCaptureCheckout, nextStep}) => {
    ...
    return (
        <>
            ...
            <Divider />
            <Typography variant="h6" gutterBottom style={{margin: '20px 0'}}>Paymant method</Typography>
            <Elements stripe={stripePromise}>
                <ElementsConsumer>
                    {(elements, stripe) => (
                        <from onSubmit={(e) => handleSubmit(e, elements, stripe)}>
                            <CardElement />
                            <br></br>
                            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                <Button variant='outlined' onClick={backStep}>Back</Button>
                                <Button type="submit" variant='contained' disabled={!stripe} color="primery">
